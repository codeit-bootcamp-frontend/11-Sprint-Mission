...
const AllItems = ({ handleSortChange, sortBy, setSortBy }) => {
  const [isOpen, setIsOpen] = useState(false); // 드롭다운 열림/닫힘 상태
  const [currentPage, setCurrentPage] = useState(1);
  const [pageSize] = useState(10); // 한 페이지당 아이템 수
  ...
  const { data, isLoading, isError, isFetching } = useQuery({
    queryKey: ["posts", currentPage], // 캐시 키는 페이지마다 다르게 설정
    queryFn: () => fetchItems(currentPage, 10), // 데이터를 가져오는 함수
  ...
  const totalPages = data ? Math.ceil(data.totalCount / pageSize) : 0; // totalCount에서 페이지 수 계산
  ...
  return (
    <div className="w-full p-2 sm:flex  flex-col gap-y-3 lg:w-2/3 lg:justify-center">
      ...
      <ReactPaginate
        ...
        marginPagesDisplayed={1} // 양쪽에 표시할 페이지 수
      ...
      />{" "}
      {isFetching && !isLoading && <div>Fetching more items...</div>}{" "}
      {/* 페이지를 바꿀 때 */}
    </div>
  );
};
