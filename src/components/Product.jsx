    ...
    <Link to={`/items/${id}`} title={`${name} 상세보기`}>
      <div className="product">
        <div className="img-wrap">
          <img
            src={image ? image : "/images/icons/ic_no_image.svg"} // 등록된 이미지가 없는 경우 대체 이미지(ic_no_image) 노출
            alt={description}
            onError={(e) => (e.target.src = "/images/icons/ic_no_image.svg")} // 이미지 로드에 에러 발생하는 경우 대체 이미지(ic_no_image) 노출
          />
        </div>
