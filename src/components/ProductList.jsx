...
function ProductList() {
  ...
  const [productsPerPage, setProductsPerPage] = useState(10); // 반응형에 따라 보여줄 Product 수를 할당할 state
  const [device, setDevice] = useState("pc"); // 반응형 ('pc' | 'tablet' | 'mobile')
   ...
   * 재사용을 위해 useCallback Hook으로 함수 캐싱
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      ...
      if (device !== currentDevice) {
        ...
        fetchProducts({
          ...
          setProducts(list);
          countTotalPage(totalCount, perPage);
        });
      }
    };
    ...
    fetchProducts({
      ...
      setProducts(list);
      countTotalPage(totalCount, initialPerPage);
    });

    window.addEventListener("resize", handleResize);
    return () => window.removeEventListener("resize", handleResize); // 언마운트: 리사이즈 이벤트 삭제
  }, [device, order, currentPage, countTotalPage]);
